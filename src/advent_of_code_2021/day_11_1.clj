...
(defn make-steps [steps matrix-hash]
  (loop [steps-made 0
         current-matrix matrix-hash
         flashes-acc 0]

    (let [flashes (+ flashes-acc (count-zeros current-matrix))]
      (if (= steps-made steps)
        flashes
        (recur (inc steps-made) (resolve (inc-global-energy current-matrix)) flashes)))))
